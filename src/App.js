...
const ECFAddress = "0x407FF7Dd648a47D2b2E037FC3d317f3b17b3477C"
...
function App() {
  ...
  async function startCoinFlip() {
    ...
    if (typeof window.ethereum !== 'undefined') {
      ...
      // call the newcoinflip function from the contract and send the wager amount


      // call newcoinflip function from contract and pass in wager amount after converting it to Ether
      ...
      // const coinFlippeth = await contract.newCoinFlip(ethers.utils.formatEther(ethers.utils.parseEther(wager)));

      // const coinFlippeth = await contract.newCoinFlip(ethers.utils.parseEther(ethers.utils.formatEther(wager)));

  
      
      // .send({
      //   value: ethers.utils.parseEther(ethers.utils.formatEther(wager).ethers.utils.parseEther(wager))
      // });


            // const coinFlippeth = await contract.newCoinFlip(ethers.utils.parseEther(wager));
          ...
          const coinFlippeth = await contract.newCoinFlip(updatedWager);


      // look up ethers.utils.formatEther(balance) in the ethersjs docs

      // this might help too https://github.com/ethers-io/ethers.js/issues/563
      ...
      console.log(`You started the wager with ${updatedWager} ETH`);
